...
def generate_complex_dummy_data():
    """Generates dummy data using a shifted Power Law."""
    ...
    for _ in range(5000000):
        ...
        pt = 20.0 + 35.0 / math.pow(1.0 - ROOT.gRandom.Uniform(0, 1), 1.0/(4.0 - 1.0))
...
def derive_complex_fit(h_pt, min_pt, max_pt, eta_str):
    """
    Fits N_tot * Normalized Shifted Power Law.
    ...
    # --- SINGLE SHIFTED POWER LAW MODEL ---
    # [0]=N_tot, [1]=alpha, [2]=p_min, [3]=p_max, [4]=k
    pl_pdf = "([0] * ((1.0-[1])*pow(x + [4], -[1])) / (pow([3] + [4], 1.0-[1]) - pow([2] + [4], 1.0-[1])))"

    if first_data_bin > b2: 
        f_fail = ROOT.TF1("f_fail", pl_pdf, min_pt, max_pt)
        f_fail.SetParameters(1.0, 4.0, min_pt, max_pt, 10.0)
    ...
    f_fit = ROOT.TF1(f"f_fit_{eta_str}", pl_pdf, true_min_pt, max_pt)
    
    init_alpha = 4.0
    init_k = 45.0 

    f_fit.SetParameters(safe_N, init_alpha, true_min_pt, max_pt, init_k)
    
    f_fit.FixParameter(0, safe_N)
    f_fit.FixParameter(2, true_min_pt)
    f_fit.FixParameter(3, max_pt)

    f_fit.SetParLimits(1, 3.5, 20.0)  # alpha bounds
    f_fit.SetParLimits(4, 20.0, 200.0) # k bounds
...
def plot_analytic_profiles(h2_prof, config_list, out_dir="./adaptive_bins"):
    ...
    for i, bin_config in enumerate(config_list):
        ...
        alpha = f_fit.GetParameter(1)
        p_min_fit = f_fit.GetParameter(2)
        p_max_fit = f_fit.GetParameter(3)
        k_fit = f_fit.GetParameter(4) 
        pow_alpha = 1.0 - alpha
        ...
        for pt_step in range(int(draw_min_pt), int(max_pt_bound), 10):
            ...
            if fit_status == 0:
                ...
                n_exp = safe_N * (num_pl / denom_pl) if denom_pl != 0 else 0.0
            else:
                denom_pl = math.pow(p_max_fit + k_fit, pow_alpha) - math.pow(p_min_fit + k_fit, pow_alpha)
                num_pl = math.pow(pt_step + width + k_fit, pow_alpha) - math.pow(pt_step + k_fit, pow_alpha)
                n_exp = safe_N * (num_pl / denom_pl) if denom_pl != 0 else 0.0
            ...
            raw_target = min(stat_unc, jer_val)
            smoothed_target = max(raw_target, max_target_so_far)
            max_target_so_far = raw_target

            g_unc.SetPoint(g_unc.GetN(), pt_step, raw_target)
...
def plot_complex_spectrum(h2_prof, config_list, out_dir="./adaptive_bins"):
    ...
    for i, bin_config in enumerate(config_list):
        ...
        #pad1.cd()
        h_spectrum.SetTitle(f"Tag Jet Spectrum |#eta| #in [{eta_min:.3f}, {eta_max:.3f}];;dN/dp_{{T}} (Events / GeV)")
        ...
        #h_spectrum.GetXaxis().SetLabelSize(0); h_spectrum.GetXaxis().SetTitleSize(0) # Make x-axis invisible
        h_spectrum.GetYaxis().SetTitleSize(0.03); h_spectrum.GetYaxis().SetLabelSize(0.03); h_spectrum.GetYaxis().SetTitleOffset(1.1); h_spectrum.GetXaxis().SetTitle("p^{tag}_{T} [GeV]");
        h_spectrum.GetXaxis().SetNoExponent(True); h_spectrum.GetXaxis().SetMoreLogLabels(True) # Better labels in x-axis
        ...
        if fit_status == 0:
            ...
            alpha = f_fit.GetParameter(1)
            k_val = f_fit.GetParameter(4)
            
            pave = ROOT.TPaveText(0.50, 0.42, 0.90, 0.70, "NDC")
            pave.SetFillStyle(0); pave.SetBorderSize(0); pave.SetTextFont(42); pave.SetTextSize(0.025)
            pave.AddText(r'Model: N_{tot} #times (p_{T} + k)^{-#alpha}')
            pave.AddText(f'N_{{tot}} = {safe_N:.2e}')
            pave.AddText(f'#alpha = {alpha:.2f} #pm {f_fit.GetParError(1):.2f}')
            pave.AddText(f'Offset (k) = {k_val:.1f} #pm {f_fit.GetParError(4):.1f}')
            pave.Draw()

        leg = ROOT.TLegend(0.50, 0.72, 0.90, 0.88)
        leg.SetFillStyle(0); leg.SetBorderSize(0); leg.SetTextFont(42); leg.SetTextSize(0.025)
        leg.AddEntry(h_spectrum, "Data Spectrum (dN/dp_{T})", "pe")
        if fit_status == 0:
            leg.AddEntry(h_band, "Fit Uncertainty (#pm 1#sigma)", "f")
            leg.AddEntry(f_fit, "Power Law Fit", "l")
        leg.Draw()
        ...
        # h_pull.GetXaxis().SetTitle("Teg p^{tag}_{T} (GeV)"); h_pull.GetXaxis().SetTitleSize(0.14)
...
def find_adaptive_uncertainty_binning(h2_prof, config_list, out_dir="./adaptive_bins"):
    ...
    for i, bin_config in enumerate(config_list):
        ...
        # --- THE FIX: Start processing exactly from the min_pt defined in config ---
        first_bin_to_process = h_pt.GetXaxis().FindBin(min_pt_limit + 1e-5) 
        last_bin_to_process = h_pt.GetXaxis().FindBin(max_pt_limit - 1e-5)

        # Force the first edge to strictly be the lower edge of min_pt_limit
        ...
        for iy in range(first_bin_to_process, last_bin_to_process + 1):
            ...
            if W_M > 1:
                ...
                raw_target = min(stat_unc, dynamic_jer_fraction)
                smoothed_target = max(raw_target, max_target_so_far)
                max_target_so_far = smoothed_target
                
                if E_M <= smoothed_target and bin_width >= min_required_width:
                    edges.append(pt_end)
                    W_M, SumWY_M, SumWY2_M = 0.0, 0.0, 0.0 
        ...
        if len(clean_edges) > 1:
            ...
            #label_complex = f"Fit (#alpha={alpha:.2f}, k={k_fit:.1f})" if fit_status == 0 else "PL Fallback Model"
            #h_rebin.SetTitle(f"MPF |#eta| #in [{eta_min:.3f}, {eta_max:.3f}] ({label_complex}, N={N:.1f}, S={S:.2f}, C={C:.3f});Tag p_{{T}} (GeV);MPF Response")
            h_rebin.SetTitle(f"MPF |#eta| #in [{eta_min:.3f}, {eta_max:.3f}];Tag p_{{T}} (GeV);MPF Response")
...
if __name__ == "__main__":
    ...
    base_config = [
        {"eta_edges": [0.0, 0.261],   "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 2000.0, "fit_min_pt":50.0},
        {"eta_edges": [0.261, 0.522], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 2000.0, "fit_min_pt":50.0},
        {"eta_edges": [0.522, 0.783], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 2000.0, "fit_min_pt":50.0},
        {"eta_edges": [0.783, 1.044], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 2000.0, "fit_min_pt":50.0},
        {"eta_edges": [1.044, 1.305], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 2000.0, "fit_min_pt":50.0},

        {"eta_edges": [1.305, 1.479], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 1500.0, "fit_min_pt":50.0},
        {"eta_edges": [1.479, 1.653], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 1500.0, "fit_min_pt":50.0},
        {"eta_edges": [1.653, 1.930], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 1500.0, "fit_min_pt":50.0},
        {"eta_edges": [1.930, 2.322], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 1500.0, "fit_min_pt":50.0},
        {"eta_edges": [2.322, 2.650], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 1000.0, "fit_min_pt":50.0},
        {"eta_edges": [2.650, 2.964], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 1000.0, "fit_min_pt":50.0},
        {"eta_edges": [2.964, 5.191], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 1000.0, "fit_min_pt":50.0},
    ]
    ...
    #     {"eta_edges": [1.479, 1.653], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 1500.0, "fit_min_pt":50.0},
    #     {"eta_edges": [1.653, 1.930], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 1500.0, "fit_min_pt":50.0},
    #     {"eta_edges": [1.930, 2.172], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 1500.0, "fit_min_pt":50.0},
    #     {"eta_edges": [2.172, 2.322], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 1500.0, "fit_min_pt":50.0},
    #     {"eta_edges": [2.322, 2.500], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 1500.0, "fit_min_pt":50.0},

    #     {"eta_edges": [2.500, 2.650], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 1000.0, "fit_min_pt":50.0},
    #     {"eta_edges": [2.650, 2.853], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 1000.0, "fit_min_pt":50.0},
    #     {"eta_edges": [2.853, 2.964], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 1000.0, "fit_min_pt":50.0},
    #     {"eta_edges": [2.964, 3.139], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 1000.0, "fit_min_pt":50.0},

    #     {"eta_edges": [3.139, 3.489], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 1000.0, "fit_min_pt":50.0},
    #     {"eta_edges": [3.489, 3.839], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 1000.0, "fit_min_pt":50.0},
    #     {"eta_edges": [3.839, 5.191], "pt_bins": pt_bins, "min_pt": 30.0, "max_pt": 1000.0, "fit_min_pt":50.0}
    # ]
    try:
    ...
    print("Drawing pT Spectra vs Shifted PL profiles...")

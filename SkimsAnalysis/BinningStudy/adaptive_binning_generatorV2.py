...
def plot_analytic_profiles(h2_prof, config_list, out_dir="./adaptive_bins"):
    ...
    for i, bin_config in enumerate(config_list):
        ...
        f_jer.SetMinimum(0.0); f_jer.SetMaximum(0.30)
        ...
        for pt_step in range(int(draw_min_pt), int(max_pt_bound), 10):
            ...
            g_unc.SetPoint(g_unc.GetN(), pt_step, 1.0 / math.sqrt(n_exp) if n_exp > 0 else 1.0)

        g_unc.SetLineColor(ROOT.kRed); g_unc.SetLineWidth(3); g_unc.SetLineStyle(2) 
        g_unc.Draw("L SAME")

        # drawn_lines = []
        # for e in json_results[i]["pt_binning"][1:-1]:
        #     l_vert = ROOT.TLine(e, 0.0, e, 0.30)
        #     l_vert.SetLineColor(ROOT.kGray+1); l_vert.SetLineStyle(3)
        #     l_vert.Draw("SAME")
        ...
        leg.AddEntry(g_unc, f"Target (Exp+PL)" if fit_status == 0 else "Target (Fallback)", "l")
        leg.Draw()
...
def plot_complex_spectrum(h2_prof, config_list, out_dir="./adaptive_bins"):
    ...
    for i, bin_config in enumerate(config_list):
        ...
        # drawn_lines = []
        # for e in json_results[i]["pt_binning"][1:-1]:
        #     l_vert = ROOT.TLine(e, spec_min, e, spec_max)
        #     l_vert.SetLineColor(ROOT.kGray+1); l_vert.SetLineStyle(3)
        #     l_vert.Draw("SAME")
        #     drawn_lines.append(l_vert)
    ...
    for i, bin_config in enumerate(config_list):
        ...
        for iy in range(first_bin_to_process, last_bin_to_process + 1):
            ...
            if W_M > 1:
                ...
                current_target_unc = 1.0 / math.sqrt(n_exp) if n_exp > 0 else 1.0
        ...
        if len(clean_edges) > 1:
            ...
            label_complex = f"Fit (b={b:.3f}, #alpha={alpha:.2f})" if fit_status == 0 else "PL Fallback Model (#alpha=4.0)"
...
if __name__ == "__main__":
    ...
    adaptive_config = [
        {"eta_edges": [0.0, 0.261], "N": 3.0, "S": 1.0, "C": 0.04, "min_pt": 30.0, "max_pt": 2000.0, "fit_min_pt": 60.0},
        {"eta_edges": [0.261, 0.522], "N": 3.0, "S": 1.0, "C": 0.04, "min_pt": 30.0, "max_pt": 2000.0, "fit_min_pt": 60.0},
        {"eta_edges": [0.522, 0.783], "N": 3.0, "S": 1.0, "C": 0.04, "min_pt": 30.0, "max_pt": 2000.0, "fit_min_pt": 60.0},
        {"eta_edges": [0.783, 1.044], "N": 3.0, "S": 1.0, "C": 0.04, "min_pt": 30.0, "max_pt": 2000.0, "fit_min_pt": 60.0},
        {"eta_edges": [1.044, 1.305], "N": 3.0, "S": 1.0, "C": 0.04, "min_pt": 30.0, "max_pt": 2000.0, "fit_min_pt": 60.0},

        {"eta_edges": [1.305, 1.479], "N": 4.0, "S": 1.0, "C": 0.05, "min_pt": 30.0, "max_pt": 1500.0, "fit_min_pt": 60.0},
        {"eta_edges": [1.479, 1.653], "N": 4.0, "S": 1.0, "C": 0.05, "min_pt": 30.0, "max_pt": 1500.0, "fit_min_pt": 60.0},
        {"eta_edges": [1.653, 1.930], "N": 4.0, "S": 1.0, "C": 0.05, "min_pt": 30.0, "max_pt": 1500.0, "fit_min_pt": 60.0},
        {"eta_edges": [1.930, 2.172], "N": 4.0, "S": 1.0, "C": 0.05, "min_pt": 30.0, "max_pt": 1500.0, "fit_min_pt": 60.0},
        {"eta_edges": [2.172, 2.322], "N": 4.0, "S": 1.0, "C": 0.05, "min_pt": 30.0, "max_pt": 1500.0, "fit_min_pt": 60.0},
        {"eta_edges": [2.322, 2.500], "N": 4.0, "S": 1.0, "C": 0.05, "min_pt": 30.0, "max_pt": 1500.0, "fit_min_pt": 60.0},

        {"eta_edges": [2.500, 2.650], "N": 6.0, "S": 1.2, "C": 0.06, "min_pt": 30.0, "max_pt": 1000.0, "fit_min_pt": 60.0},
        {"eta_edges": [2.650, 2.853], "N": 6.0, "S": 1.2, "C": 0.06, "min_pt": 30.0, "max_pt": 1000.0, "fit_min_pt": 60.0},
        {"eta_edges": [2.853, 2.964], "N": 6.0, "S": 1.2, "C": 0.06, "min_pt": 30.0, "max_pt": 1000.0, "fit_min_pt": 60.0},
        {"eta_edges": [2.964, 3.139], "N": 6.0, "S": 1.2, "C": 0.06, "min_pt": 30.0, "max_pt": 1000.0, "fit_min_pt": 60.0},

        {"eta_edges": [3.139, 3.489], "N": 8.0, "S": 1.5, "C": 0.08, "min_pt": 30.0, "fit_min_pt": 60.0},
        {"eta_edges": [3.489, 3.839], "N": 8.0, "S": 1.5, "C": 0.08, "min_pt": 30.0, "fit_min_pt": 60.0},
        {"eta_edges": [3.839, 5.191], "N": 8.0, "S": 1.5, "C": 0.08, "min_pt": 30.0, "fit_min_pt": 60.0}
    ]
    ...
    plot_analytic_profiles(h2_prof, config_list=adaptive_config)
    
    print("Drawing pT Spectra vs Complex Fit (Exp + PL) profiles...")
    plot_complex_spectrum(h2_prof, config_list=adaptive_config)
